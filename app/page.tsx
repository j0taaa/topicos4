...
export default function Home() {
  ...
  return <main className="presentation-lab">
    <header className="presentation-header">
      <div><h1>{slides[0].title}</h1><p>{slides.length} slides · artigos, questões e discussão</p></div>
    ...
    <nav className="presentation-controls" aria-label="Controles da apresentação">
      ...
      <div className="slide-status"><b>{index + 1} / {slides.length}</b><span>{current.title}</span>{current.maxStep > 0 && <small>{`diagrama ${step + 1} de ${current.maxStep + 1}`}</small>}</div>
